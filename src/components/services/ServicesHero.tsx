...
interface ServicesHeroSettings {
  title: string;
  subtitle: string;
...
export default function ServicesHero() {
  const [settings, setSettings] = useState<ServicesHeroSettings>({
    title: 'Our Services',
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const response = await fetch(API_ENDPOINTS.services.hero);
        if (response.ok) {
          const data = await response.json();
          if (data.results && data.results.length > 0) {
            const heroData = data.results[0];
            setSettings({
              title: heroData.title || 'Our Services',
              subtitle: heroData.subtitle || 'Comprehensive Real Estate Solutions',
              background_url: heroData.background_url || 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&q=80&w=1920',
            });
          }
        }
      } catch (error) {
        console.error('Failed to fetch services hero settings:', error);
    ...
    <section className="relative min-h-[300px] sm:min-h-[347px] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <LazyImage
          ...
          fallbackSrc="https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&q=80&w=1920"
