...
export default function PropertyFloorPlan({ property }: { property: PropertyData }) {
  const getFloorPlanImage = (image: string | null): string | null => {
    if (!image) return null;
    if (image.startsWith('http')) return image;
    return `${process.env.NEXT_PUBLIC_API_URL}${image}`;
  };

  const floorPlanSrc = property.floor_plan_url || getFloorPlanImage(property.floor_plan);

  return (
    <div className="bg-white">
      <h2 className="text-2xl font-bold text-[#1a1a1a] mb-8 font-sans">Floor Plan</h2>

      <motion.div
        className="relative w-full rounded-sm overflow-hidden shadow-lg border border-gray-100"
      ...
      >
        {floorPlanSrc ? (
          <>
            <img
              src={floorPlanSrc}
              alt="Architectural floor plan blueprints"
          ...
          <div className="w-full h-64 bg-[#f8f6f1] flex flex-col items-center justify-center gap-3 border border-dashed border-[#d4c4a8]">
            <svg className="w-12 h-12 text-[#c1a478]/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
            </svg>
            <p className="text-[#7C7A70] text-sm font-medium">Floor plan not available</p>
            <p className="text-[#7C7A70]/60 text-xs">Upload a floor plan in the admin panel</p>
          </div>
        )}
      </motion.div>
